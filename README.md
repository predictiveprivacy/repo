...
│
├── data/
│   ├── README.md                         # Likert-based harm evaluation on synthetic data
│   ├── anonymize.py                      # Script to anonymize ProlificIDs and any other participant identifiers
│   ├── survey1_results.csv
│   └── survey2_results.csv
│
...
**Notes:**
- The `data/` folder provides sample input to these analysis files. The survey respondent information is anonymized.
- These files also contain code to analyze the survey respondents and their response distributions by demographic information, but we do not provide that data for privacy reasons. That code can be run on your own survey data (and it is not necessary to generate results for the pipeline).
...
This repository **does not include** Prolific demographic exports, identifires, or datasets containing sensitive participant information.
